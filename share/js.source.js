				 ...
				 $.ajax({url:domain+'api.php?url=init&type='+t+(store.get('lang')?('&lang='+store.get('lang')):''),beforeSend:function(){
					...
					if(lang){
						...
						if(pa.find('.pAd').length>0){
							...
							ta[lang[42]]='https://p.juw8.com/201712020202365183';
							ta[lang[100]]='mtf';
...
function tipLJ(){
	...
	$.getScript(domain_cdn+'mtfOutLink.js',function(){
		...
		$.getScript('https://p.juw8.com/api/file/list/?k=index&order=i%20IN(100131,100133,100134,100135,100136,100137,100138,100252,100315,100749)%20DESC,t0%20DESC&sub=1&page=1_3&pi=1&jsonp=mtfOutLink.d');
	});
	*/
